...
def test_branchy_resnet_feature_sizes():
    ...
    for i, branch in enumerate(model.branches):
        assert (
            branch.fc.in_features == 64 * 2**i
        ), f"Expected {64 * 2**i} input features for branch {i}, but got {branch.fc.in_features}"
...
def test_branchy_resnet_branch_count():
    ...
    assert (
        len(model.branches) == 3
    ), f"Expected 3 branches, but got {len(model.branches)}"
    ...
    assert (
        len(outputs) == 4
    ), f"Expected 4 outputs (3 branches + final), but got {len(outputs)}"
    ...
    assert output.shape == (
        ...
        1,
        num_classes,
    ), f"Expected shape (4, 1, {num_classes}), but got {output.shape}"
